...
def main():
    ...
    with wandb.init(config=None, dir=checkpoint_folder):
        ...
        args.max_physical_batch_size = 32
...
def main_worker(args, checkpoint_folder):
    ...
    for m in list(best_metric_for_epoch_et.keys())[:1]:
        print('{} {:.4f} '.format(m, best_metric_for_epoch_et[m]), end=' ')
    print("micro_auc {:.4f}".format(best_metric_val), end='  ')
...
project_name = "paper_3_moco_model_and_datasets"
sweep_configuration = {
    'method': 'grid',
    'metric': {
        'goal': 'maximize',
    ...
    'parameters': {
        'pretrain_para': {"values": ['cheXpert']},  # choose parameters
        "fine_tuning": {'values': ['full', 'linear']},  # training methods
...
#################################################
# python main_lincls_3_resnet18_4.py -a resnet18 --epochs 50 --binary --workers 0 --gpu 0
