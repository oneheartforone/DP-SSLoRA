...
###################################################################
# In this code, we study in-depth for the Privacy-Utility trade-off
# DP version.
######################################################################
...
import wandb
# prohibit wandb
# os.environ['WANDB_MODE'] = 'offline'  # 0.13
# os.environ['WANDB_MODE'] = 'dryrun'   # 0.9.7
...
def main():
    ...
    with wandb.init(config=None, dir=checkpoint_folder):
        ...
        args.crop = 320
        args.train_aug = True
        args.model_init = "zero"
        args.schedule = [30, 38]
        args.batch_size = 256

        # 需要测试能否正确选择预训练文件，以及能否正确赋给模型
        # 需要测试能够正确选择full or linear
        # 需要测试能否正确在DP 和 nonDP之间切换
        ...
        if args.pretrain_para == "imagenet":
            ...
            print("Use CheXpert pretrain parameters. ")
        ...
        if args.dataset == "shenzhen":
            args.train_data = r"../../data\shenzhenXray\chexpert_like_1"
            args.val_data = r"../../data\shenzhenXray\val_path"
            args.test_data = r"../../data\shenzhenXray\test_path"
            args.delta = 1e-3
            ...
            args.train_data = r"../../data\covid_Chest_X-Ray_Images11G\balanced_11G\balanced"
...
def main_worker(args, checkpoint_folder):
    ...
    if args.binary:
        best_metrics.update({'auc': {'func': 'compute_auc_binary', 'format': ':6.3f', 'args': []}})
    ...
    if args.evaluate:
        evaluator.evaluate('valid', 0, 0)
        evaluator.evaluate('test', 0, 0)
        return

    evaluator.evaluate('test', 0, 0)  # before training
    ...
    for i, epoch in enumerate(range(args.start_epoch, args.epochs)):
        ...
        wandb_dict.update({"Train ACC@1": cur_train_metrics["acc@1"].avg, "Train AUC": cur_train_metrics["auc"].avg,
                           "Train losses": cur_train_losses.avg, "epsilon": args.epsilon})

        metric_meters_ev, valid_losses, best_metric_for_epoch_ev = evaluator.evaluate('valid', epoch, epoch)
        wandb_dict.update({"Valid Acc": metric_meters_ev["acc@1"].avg, "Valid AUC": metric_meters_ev["auc"].avg})

        metric_meters_et, test_losses, best_metric_for_epoch_et = evaluator.evaluate('test', 0, epoch)  # 0, But we should technically not optimize for this
        wandb_dict.update({"Test Acc@1": metric_meters_et["acc@1"].avg, "Test AUC":  metric_meters_et["auc"].avg}) #?

        # test acc and auc
        is_best = evaluator.metric_best_vals[args.best_metric] > best_metric_val   # args.best_metric default "acc@1"
        best_metric_val = max(best_metric_val, evaluator.metric_best_vals[args.best_metric])  # acc@1

        # 测试 不一定需要
        wandb_dict.update({"Best Test Acc@1": best_metric_for_epoch_et["acc@1"], "Best Test AUC": best_metric_for_epoch_et["auc"]})

        if is_best:
            wandb.run.summary["all_best_accuracy"] = list(best_metric_for_epoch_et.keys())[0]
            wandb.run.summary["all_best_auc"] = list(best_metric_for_epoch_et.keys())[1]


        if is_best == True or epoch == args.epochs-1:  # effect running speed
    ...
    evaluator.evaluate('test', epoch + 1, epoch)

    print("Best test acc and the corresponding auc and epoch.")
    for m in list(best_metric_for_epoch_et.keys())[:2]:
        print('{} {:.3f} '.format(m, best_metric_for_epoch_et[m]), end=' ')
    print(list(best_metric_for_epoch_et.keys())[-1], int(best_metric_for_epoch_et[list(best_metric_for_epoch_ev.keys())[-1]]))
...
def train(train_loader, model, criterion, optimizer, epoch, args, best_metrics, privacy_engine):
    ...
    if privacy_engine is not None and args.fine_tuning == 'linear':
...
project_name = "paper_3_moco_model_and_datasets_fixed_epsilon"
sweep_configuration = {
    'method': 'grid',
    'metric': {
        'goal': 'maximize',
    ...
    'parameters': {
        ...
        "fine_tuning": {'values': ['linear']},  # training methods
        'dp': {'values': [True, False]},  # Whether to use DP
...
if __name__ == '__main__':
    wandb.agent(sweep_id, function=main, count=16)  # 5nthr6d4
    wandb.finish()
...
# python main_lincls_3_resnet18_4.py -a resnet18 --epsilon 1.0 --epochs 40 --binary --workers 0 --gpu 0
