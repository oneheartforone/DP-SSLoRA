...
def move_chexpert_single_target(root_folder, destination_root, disease_name):
    ...
    cwd = os.getcwd()
    root_path = Path(cwd) / Path(root_folder)
    ...
    def move_to_category(category, folder, df):
        ...
        for i, row in tqdm(df.iterrows()):
            ...
            if not dst_fname.exists():
                os.symlink(str(desired_path), dst_fname)   # Create a symbolic link pointing to src named dst
            else:
                # Do nothing for now
                pass
...
if __name__ == '__main__':
    ...
    if sys.argv[1] == 'montgomery':
        ...
        # python reorganize_files.py  shenzhen "D:\yhc\MoCo-CXR-main\data\shenzhenXray"  "D:\yhc\MoCo-CXR-main\data\shenzhenXray\images\classed"
        move_shenzhen(sys.argv[2], sys.argv[3])
    elif sys.argv[1] == 'chexpert_single':
        # root_folder, destination_root, disease name
        # python  moco_pretraining/scripts/reorganize_files.py chexpert_single ./data/CheXpert-v1.0-small ./data/chexpert_single  "Pleural Effusion"
