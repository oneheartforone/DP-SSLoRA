...
class MoCo(nn.Module):
    ...
    def __init__(self, base_encoder, dim=128, K=65536, m=0.999, T=0.07, gn=False, mlp=False, pretrained=False):
        ...
        if pretrained:
            ...
            if gn:  # if you want change BN to GN
                self.encoder_q = ModuleValidator.fix(base_encoder(num_classes=dim))
                self.encoder_k = ModuleValidator.fix(base_encoder(num_classes=dim))
            else:
