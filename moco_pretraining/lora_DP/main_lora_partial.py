...
# os.environ['WANDB_MODE'] = 'disabled'   # no run
...
def main():
    ...
    with wandb.init(config=None, dir=checkpoint_folder):
        ...
        args.crop = args.img_size
        args.partial = config.partial
        ...
        args.noise_multiplier = config.noise
        args.batch_size = config.batch_size
        if args.batch_size < 64:
            args.max_physical_batch_size = 16
        else:
        ...
        if args.dataset == "RSNA":
            if args.partial == 0.01:
                args.train_data = r"../../data\RSNA\Split\partial\001"
                args.delta = 1e-2
            elif args.partial == 0.1:
                args.train_data = r"../../data\RSNA\Split\partial\025"
                args.delta = 1e-3
            elif args.partial == 0.25:
                args.train_data = r"../../data\RSNA\Split\partial\01"
                args.delta = 1e-4
            elif args.partial == 0.5:
                args.train_data = r"../../data\RSNA\Split\partial\05"
                args.delta = 1e-4
            else:
            ...
            if args.partial == 0.01:
                args.train_data = r"../../data\covid_Radiography_Dataset08G\yin\partial\001"
                args.delta = 1e-3
            elif args.partial == 0.1:
                args.train_data = r"../../data\covid_Radiography_Dataset08G\yin\partial\010"
                args.delta = 1e-3
            elif args.partial == 0.25:
                args.train_data = r"../../data\covid_Radiography_Dataset08G\yin\partial\025"
                args.delta = 1e-3
            elif args.partial == 0.5:
                args.train_data = r"../../data\covid_Radiography_Dataset08G\yin\partial\050"
                args.delta = 1e-4
            else:
            ...
            if args.partial == 0.01:
                args.train_data = r"../../data\covid_Chest_X-Ray_Images11G\partial\001"
                args.delta = 1e-4
            elif args.partial == 0.1:
                args.train_data = r"../../data\covid_Chest_X-Ray_Images11G\partial\010"
                args.delta = 1e-4
            elif args.partial == 0.25:
                args.train_data = r"../../data\covid_Chest_X-Ray_Images11G\partial\025"
                args.delta = 1e-4
            elif args.partial == 0.5:
                args.train_data = r"../../data\covid_Chest_X-Ray_Images11G\partial\050"
                args.delta = 1e-4
            else:
...
def main_worker(args, checkpoint_folder):
    ...
    cm, cr, auc, all_output_best, all_gt_best = [], [], [], [], []  # Confusion Matrix, classification Report, auc, output, target
...
project_name = "paper_3_moco_LoRA_4_multimodel"
sweep_configuration = {
    'method': 'grid',
    'metric': {
    'goal': 'maximize',
    ...
    'parameters': {
        ...
        # specific search lora rank
        'batch_size': {'values': [256,]},
        "fine_tuning": {'values': ['lora']},  # training method
        'dp': {'values': [True]},  # Whether to use DP
        'img_size': {'values': [320]},
        'lora_rank': {'values': [5, ]},  # Whether to use DP
        'aug': {'values': [True]},  # train augment
        'lr_decay': {'values': [True]},  # lr decay
        'pretrain_para': {"values": ["cheXpert128",]},  # pretrain model
        'model_init': {"values": ["normal"]},  # linear layer, zero initial vs. normal initial
        'lr': {'values': [0.001,]},  # learning rate
        'dataset': {'values': ["covid08G", "covid11G", "RSNA"]},
        'noise': {'values': [1.]},  # noise multiplier
        'partial': {'values': [0.01, 0.1, 0.25, 0.5, 1.0,]},  # noise multiplier
