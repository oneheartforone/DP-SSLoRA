...
def main_worker(args, checkpoint_folder):
    ...
    # total_params = sum(p.numel() for p in model.parameters()) / 1024
    ...
    if args.optimizer == 'sgd':
        optimizer = torch.optim.SGD(model.parameters(), args.lr,
                                    momentum=args.momentum,) #params_to_optimize,
    ...
    for i, epoch in enumerate(range(args.start_epoch, args.epochs)):
        ...
        # train for one epoch
        if args.dp:
            cur_train_metrics, cur_train_losses, = train(train_loader, model, criterion, optimizer, epoch, args, best_metrics, privacy_engine)
...
# project_name = "paper_3_moco_LoRA_fixedepsilon_augT"
project_name = "paper_3_moco_LoRA_fixedepsilon"
sweep_configuration = {
    'method': 'grid',
    'metric': {
    'goal': 'maximize',
    ...
    'parameters': {
        ...
        'epsilon': {'values': [2., 4, 6, 8, 10]},  # epsilon
...
# python main_lora_fixedepsilon.py -a resnet18 --epochs 50 --workers 0 --gpu 0
