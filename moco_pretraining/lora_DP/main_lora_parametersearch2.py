...
wandb.sdk.wandb_run._show_warning_context = True
...
def main():
    ...
    with wandb.init(config=None, dir=checkpoint_folder):
        ...
        args.loralayer = config.loralayer
...
def main_worker(args, checkpoint_folder):
    ...
    # "Conv2d" BasicBlock fc
    tmp_loralayer = {0: "model.layer1", 1: "model.layer2", 2: "model.layer3", 3: "model.layer4"}
    model_lora_params = []

    if args.loralayer <= 4:
        for i in range(args.loralayer):
            model_lora_params_, _ = inject_trainable_lora_extended(eval(tmp_loralayer[i]), target_replace_module={"BasicBlock"}, r=args.lora_rank)
            model_lora_params += model_lora_params_
...
# project_name = "paper_3_moco_LoRA_randomSearch"
project_name = "test"
sweep_configuration = {
    'method': 'grid',
    'metric': {
    ...
    'parameters': {
        ...
        # 'lora_rank': {'values': [3, 5, 8, 10, 15]},  # Whether to use DP
        # 'aug': {'values': [True, False]},  # train augment
        # 'dp': {'values': [True, False]},  # Whether to use DP
        # 'noise': {'values': [1, 5, 10, 15]},  # noise multiplier
        # 'lora_rank': {'values': [3, 5, 8, 10, 15]},  # Whether to use lora
        # 'loralayer': {'values': [4, 3, 2, 1]}  # ablation for lora layers,
        ...
        'img_size': {'values': [320]},
        'lora_rank': {'values': [5, ]},  # rank
        'aug': {'values': [False]},  # train augment
        ...
        'dataset': {'values': ["RSNA"]},  # choose dataset
        'noise': {'values': [1, 5, 10, 15]},  # noise multiplier
